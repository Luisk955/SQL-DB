...
AS
	...
	UPDATE USERS SET NAME=@P_NAME,LAST_NAME=@P_LAST_NAME,PHONE_NUMBER=@P_PHONE_NUMBER,BIRTHDATE=@P_BIRTHDATE,STATUS=@P_STATUS,PASSWORD=@P_PASSWORD, BALANCE=@P_BALANCE, PAYPAL_USER=@P_PAYPAL_USER, EMAIL=@P_EMAIL, CREATION_DATE=@P_CREATION_DATE,CHAT_ID=@P_CHAT_ID WHERE ID=@P_ID;
...
CREATE PROCEDURE RET_VISIBLE_ORDER_USER_PR
	@P_ID int
as
	Select  O.ID ID, US.NAME BUYER,L.NAME RESTAURANT,D.NAME DRIVER,DIR.COORDINATE_X LATITUDE, DIR.COORDINATE_Y LONGITUDE,U.Name STATUS,O.Start_Time START_TIME,L.LOCATION REST_LOC from orders O
	Join UTILS U on O.STATUS=U.Id
	JOIN USERS US ON O.ID_BUYER=US.ID
	JOIN LOCAL L ON O.ID_SELLER=L.ID
	left JOIN USERS D ON O.ID_DRIVER=D.ID
	left JOIN DIRECTION DIR ON O.ID_LOCATION=DIR.ID
...
GO
CREATE PROCEDURE RET_ALL_PENDING_DRIVER_ORDER_PR
AS
	SELECT O.ID ID, L.NAME RESTAURANT,DIR.COORDINATE_X LATITUDE,DIR.COORDINATE_Y LONGITUDE, O.START_TIME START_TIME, L.LOCATION REST_LOC FROM ORDERS O
	LEFT JOIN LOCAL L ON O.ID_SELLER = L.ID
	LEFT JOIN DIRECTION DIR ON O.ID_LOCATION=DIR.ID
	WHERE O.STATUS='ordPenD'
GO
CREATE PROCEDURE UPD_ORDER_DRIVER_PR
	@P_ID INT,
	@P_ID_DRIVER INT
AS
	UPDATE ORDERS SET ID_DRIVER=@P_ID_DRIVER,STATUS='ordPen' WHERE ID=@P_ID
GO
CREATE PROCEDURE UPD_ORDER_STATUS_PR
	@P_ID INT,
	@P_STATUS VARCHAR(255)
AS
	UPDATE ORDERS SET STATUS=@P_STATUS WHERE ID=@P_ID
GO
CREATE PROCEDURE RET_LOCAL_OWNER_ID_PR
	@P_ID INT
AS
	SELECT U.ID ID,U.NAME NAME,U.LAST_NAME LAST_NAME,U.PASSWORD PASSWORD, U.STATUS STATUS,U.PHONE_NUMBER PHONE_NUMBER,U.BIRTHDATE BIRTHDATE,U.BALANCE BALANCE,U.PAYPAL_USER PAYPAL_USER,U.EMAIL EMAIL,U.CREATION_DATE CREATION_DATE,R.NAME TYPE,U.CHAT_ID CHAT_ID FROM USERS U
	left JOIN ROLEXUSER RXU ON U.ID=RXU.ID_USER
	left JOIN ROLE R ON RXU.ID_ROLE = R.ID
	left JOIN USERXLOCAL UL ON U.ID = UL.ID_USER
	WHERE UL.ID_LOCAL=@P_ID;
GO
